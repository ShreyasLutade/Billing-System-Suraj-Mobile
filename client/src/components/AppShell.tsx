  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        ...
        <div className="absolute -right-16 top-40 h-80 w-80 rounded-full bg-ink-900/10 blur-3xl" />
      </div>

      <header className="fixed inset-x-0 top-0 z-40 border-b border-white/50 bg-white/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 pr-[4.5rem] sm:px-6 sm:pr-24">
          <Link
            to="/"
          ...
          <nav className="hidden items-center gap-1 rounded-2xl border border-ink-100/80 bg-white/70 p-1 md:flex">
            {links.map((link) => (
              <NavLink
                key={link.to}
                to={link.to}
        ...
        <button
          type="button"
          className="btn-secondary absolute right-4 top-1/2 -translate-y-1/2 px-3 py-2 sm:right-6"
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                clsx(
                  ...
                  isActive ? "bg-ink-900 text-white" : "text-ink-500",
                )
              }
            >
              <link.icon className="h-4 w-4" />
